...
#sa = amplitude of the stress wave/alternating stress
sa = (smax - smin)/2
...
#Corrected alternating stress/Goodman Diagram
snf = sa/(1-(sm/su))
...
Z1 = -(0.8-meanD)/np.sqrt((sdDc**2)+meanD*((sd/nf)**2))
Z2 = -(1.2-meanD)/np.sqrt((sdDc**2)+meanD*((sd/nf)**2))
...
plt.plot(a1,cdf, label= 'Critical Damage = 1', color='#008000', linewidth=1)
plt.plot(a1,cdf1, label= 'Critical Damage = 0.8', color='#FF0000', linewidth=1)
plt.plot(a1,cdf2, label= 'Critical Damage = 1.2', color='#FFFF00', linewidth=1)
